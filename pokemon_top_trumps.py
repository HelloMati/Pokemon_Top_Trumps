...
def random_ability(): #takes a random ability for the pokemon
    pokemon_ability = random.randint(1,151)
...
def run(c=0,d=0): #added two parameters c and d as counters
    ...
    my_ability = random_ability()
    print(f"Your pokemon has the {my_ability['name']} ability")
    stat_choice = input('Which stat do you want to use? (id, height, weight, order) ') #player chooses the stat their pokemon would compete with
    opponent_pokemon = random_pokemon()
    print(f"The opponent chose {opponent_pokemon['name']}")
    opponent_ability = random_ability()
    print(f"Your opponent has {opponent_ability['name']} ability")
